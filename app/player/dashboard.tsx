...
export default function PlayerDashboard() {
  ...
  // Porteros disponibles filtrados por ciudad registrada del usuario
  const ciudadUser = currentUser?.ciudad || "";
  ...
  const confirmPayment = async (svcId: string) => {
    Alert.alert(
      "Confirmar pago",
      "¿Confirmas que realizaste el pago al portero?",
      [
        { text: "No", style: "cancel" },
        {
          text: "Sí, pagué", onPress: async () => {
            try {
              await updateService(svcId, { playerPaymentConfirmed: true } as any);
              Alert.alert("✅", "Pago confirmado. Ahora puedes calificar al portero.");
            } catch (e: any) {
