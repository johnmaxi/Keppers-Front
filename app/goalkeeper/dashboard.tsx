...
export default function GoalkeeperDashboard() {
  ...
  const { currentUser, services, updateService, addOffer, updateOffer, logout } = useAppStore();
  ...
  const available = services.filter(
    (s) => s.status === "pending" && (() => {
      // Solo ver servicios de mi ciudad
      if (currentUser?.ciudad && s.ciudad !== currentUser.ciudad) return false;
      const myOffer = (s.ofertas || []).find((o) => o.gkId === currentUser?.id);
  ...
  const confirmed = services.filter(
    (s) => ["confirmed", "in_progress", "completed"].includes(s.status) &&
      (s.ofertas || []).find((o) => o.id === s.acceptedOffer && o.gkId === currentUser?.id),
  );
  ...
  const endService = (svcId: string) => {
    Alert.alert("Finalizar servicio", "¿Confirmas que el partido terminó?", [
      ...
      { text: "Finalizar", onPress: () => {
          updateService(svcId, { status: "completed", completedAt: new Date().toISOString() } as any);
  ...
  const confirmPaymentReceived = async (svcId: string) => {
    const svc = services.find((s) => s.id === svcId);
    if (!svc) return;
    const commission = Math.round((svc.total || 0) * 0.15);
    const netAmount  = (svc.total || 0) - commission;
    Alert.alert(
...
Se descontará el 15% de comisión ($${commission.toLocaleString()} COP) para Keepers.`,
      [
        ...
        {
          text: "Sí, recibí el pago", onPress: async () => {
            try {
  ...
  const cancelService = (svcId: string) => {
    const svc = services.find((s) => s.id === svcId);
    if (!svc) return;
    const penalty = Math.round((svc.total || 0) * 0.15);
    Alert.alert(
      "Cancelar servicio",
      `¿Seguro que quieres cancelar este servicio?

⚠️ Se te descontará el 15% de comisión ($${penalty.toLocaleString()} COP) por cancelación.`,
      [
        { text: "No", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert("Cancelado", "El servicio fue cancelado. Se aplicó la penalidad del 15%.");
            } catch (e: any) {
      ...
      <View style={styles.root}>
        ...
        <View style={styles.blockedBox}>
          {regStatus === "pending" ? (
            <>
              <Text style={styles.blockedEmoji}>⏳</Text>
              <Text style={styles.blockedTitle}>Registro en revisión</Text>
              <Text style={styles.blockedText}>{"Tu registro está siendo revisado.\n\nRecibirás notificación al ser aprobado.\nMáximo 24 horas."}</Text>
            </>
          ) : (
            <>
              <Text style={styles.blockedEmoji}>❌</Text>
    ...
    <View style={styles.root}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerRight}>
          <View style={styles.roleBadge}><Text style={styles.roleBadgeText}>🧤 Portero</Text></View>
          <TouchableOpacity onPress={handleLogout} style={styles.logoutBtn}>
            <Text style={styles.logoutText}>Salir</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => router.push("/profile" as any)} style={styles.profileBtn}>
            {currentUser?.photoURL
              ...
              : <Text style={styles.profileInitial}>{currentUser?.nombre?.charAt(0) || "?"}</Text>
            }
          </TouchableOpacity>
        </View>
      </View>
      ...
      <View style={styles.statsRow}>
        {([["Solicitudes", available.length, "#00ff87"], ["Mis ofertas", myOffers.length, "#ffa500"], ["Activos", activeSvcs, "#00aaff"]] as const).map(([l, v, c]) => (
        ...
        {TABS.map((t) => (
          <TouchableOpacity key={t.key} style={[styles.tab, tab === t.key && styles.tabActive]} onPress={() => setTab(t.key)}>
            <Text style={[styles.tabText, tab === t.key && styles.tabTextActive]}>
              {t.emoji} {t.label}{t.key === "confirmed" && activeSvcs > 0 ? ` (${activeSvcs})` : ""}
            </Text>
          </TouchableOpacity>
        ))}
      </View>
      ...
      <ScrollView style={styles.body} contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
        ...
        {tab === "available" && (
          <View>
            <Text style={styles.sectionSub}>Envía oferta directa o contraoferta</Text>
            {available.length === 0 && (
              <View style={styles.empty}><Text style={styles.emptyEmoji}>🔍</Text><Text style={styles.emptyText}>Sin solicitudes disponibles</Text></View>
            )}
            {available.map((svc) => {
              const f   = gf(svc.id);
              ...
              return (
                <View key={svc.id} style={styles.card}>
                  <TouchableOpacity onPress={() => setExpanded(exp ? null : svc.id)}>
                    <View style={styles.rowBetween}>
                      <View>
                        <Text style={styles.svcTitle}>{svc.tipoPartido}</Text>
                        <Text style={styles.svcSub}>📍 {svc.ciudad} · {svc.cancha}</Text>
                        <Text style={styles.svcSub}>🕐 {svc.fecha} {svc.hora} · {svc.horas}h</Text>
                      </View>
                      <View style={{ alignItems: "flex-end" }}>
                        <Text style={styles.svcTotal}>${svc.total.toLocaleString()}</Text>
                        <Text style={styles.svcPago}>{svc.medioPago}</Text>
                        <Text style={styles.expandHint}>{exp ? "▲ cerrar" : "▼ responder"}</Text>
                      </View>
                    </View>
                    {svc.nota ? <Text style={styles.svcNota}>"{svc.nota}"</Text> : null}
                  </TouchableOpacity>

                  {exp && (
                    <View style={styles.offerForm}>
                      <View style={styles.modeRow}>
                        {([["offer","Oferta directa","#00ff87"],["counter","Contraoferta","#00aaff"]] as const).map(([mode,label,color]) => (
                          <TouchableOpacity key={mode} style={[styles.modeBtn, f.mode===mode && { borderColor:color, backgroundColor:`${color}12` }]}
                            onPress={() => sf(svc.id,{mode})}>
                            <Text style={[styles.modeBtnText, f.mode===mode && { color }]}>{label}</Text>
                          </TouchableOpacity>
                        ))}
                      </View>
                      ...
                      {isC && (
                        <>
                          ...
                          <View style={styles.horasRow}>
                            {[1,2,3,4,5].map((h) => (
                              <TouchableOpacity key={h} style={[styles.horaBtn, f.horas===h && styles.horaBtnActive]}
                                onPress={() => sf(svc.id,{horas:h})}>
                                <Text style={[styles.horaBtnText, f.horas===h && styles.horaBtnTextActive]}>{h}h</Text>
                              </TouchableOpacity>
                            ))}
                          </View>
                        </>
                      )}
                      <Text style={styles.label}>{isC ? "JUSTIFICACIÓN" : "MENSAJE (OPCIONAL)"}</Text>
                      <TextInput style={[styles.input,{height:70}]} multiline placeholderTextColor="#444"
                        placeholder={isC ? "Ej. Para horario nocturno cobro un poco más..." : "Ej. Puntual, experiencia en este formato..."}
                        value={f.msg} onChangeText={(v) => sf(svc.id,{msg:v})} />
                      <Text style={styles.totalPreview}>
                        Tu oferta total: <Text style={{color:isC?"#00aaff":"#00ff87",fontWeight:"800"}}>
                          ${(parseInt(f.tarifa)||BASE).toLocaleString()} COP
                        </Text>
                      </Text>
                      <TouchableOpacity style={[styles.btnPrimary, isC && {backgroundColor:"#00aaff"}]}
                        onPress={() => sendOffer(svc.id)}>
                        <Text style={styles.btnPrimaryText}>{isC ? "Enviar contraoferta" : "Enviar oferta"}</Text>
                      </TouchableOpacity>
                    </View>
                  )}
                </View>
        ...
        {tab === "myoffers" && (
          <View>
            <Text style={styles.sectionSub}>Estado de tus ofertas enviadas</Text>
            {myOffers.length === 0 && (
              <View style={styles.empty}>
                <Text style={styles.emptyEmoji}>🔔</Text>
                <Text style={styles.emptyText}>Sin ofertas enviadas</Text>
                <TouchableOpacity style={styles.btnPrimary} onPress={() => setTab("available")}>
            ...
            {myOffers.map((svc) => {
              ...
              const stC  = acc ? "#00ff87" : my?.status === "rejected" ? "#ff4757" : isC ? "#00aaff" : "#ffa500";
              const stL  = acc ? "✓ Aceptado" : my?.status === "rejected" ? "Rechazado" : isC ? "Contraoferta" : "⏳ Pendiente";
              return (
                <View key={svc.id} style={styles.card}>
                  <View style={styles.rowBetween}>
                    <View>
                      <Text style={styles.svcTitle}>{svc.tipoPartido} · {svc.ciudad}</Text>
                      <Text style={styles.svcSub}>{svc.cancha} · {svc.fecha} {svc.hora}</Text>
                    </View>
                    <Text style={[styles.statusTag, { color: stC }]}>{stL}</Text>
                  </View>
                  <Text style={[styles.svcTotal, { color: isC ? "#00aaff" : "#00ff87" }]}>
                    ${(my?.counterAmount || my?.amount || BASE).toLocaleString()} COP
                  </Text>
                  {my?.status === "rejected" && (
                    <TouchableOpacity style={styles.btnReofert}
                      onPress={() => { setTab("available"); }}>
                      <Text style={styles.btnReofertText}>🔄 Volver a ofertar</Text>
                    </TouchableOpacity>
                  )}
                </View>
              );
        ...
        {tab === "confirmed" && (
          <View>
            <Text style={styles.sectionSub}>Gestiona el ciclo de vida de tus servicios</Text>
            {confirmed.length === 0 && (
              <View style={styles.empty}><Text style={styles.emptyEmoji}>🚩</Text><Text style={styles.emptyText}>Sin servicios confirmados</Text></View>
            )}
            {confirmed.map((svc) => {
              const my       = svc.ofertas.find((o) => o.gkId === currentUser?.id);
              ...
              return (
                <View key={svc.id} style={[styles.card, canEnd && { borderColor:"rgba(0,255,135,.4)" }]}>
                  <View style={styles.rowBetween}>
                    <Text style={styles.svcTitle}>{svc.tipoPartido}</Text>
                    <Text style={[styles.statusTag, { color: statusColor[svc.status] }]}>{statusLabel[svc.status]}</Text>
                  </View>
                  <View style={styles.infoGrid}>
                    {([["Ciudad",svc.ciudad],["Cancha",svc.cancha],["Fecha",svc.fecha],["Hora",svc.hora],["Duración",`${svc.horas}h`],["Jugador",svc.playerName]] as const).map(([k,v]) => (
                  ...
                  <View style={styles.rowBetween}>
                    ...
                    <Text style={styles.svcTotal}>${total.toLocaleString()}</Text>
                  </View>

                  {canStart && (
                    <View style={styles.actionBox}>
                      <Text style={styles.actionHint}>Presiona cuando estés listo en la cancha.</Text>
                      <TouchableOpacity style={styles.btnStart} onPress={() => startService(svc.id)}>
                        <Text style={styles.btnStartText}>▶  Iniciar servicio</Text>
                      </TouchableOpacity>
                      <TouchableOpacity style={[styles.btnCancel, { marginTop: 8 }]} onPress={() => cancelService(svc.id)}>
                        <Text style={styles.btnCancelText}>Cancelar servicio</Text>
                      </TouchableOpacity>
                    </View>
                  )}
                  ...
                  {canEnd && (
                    <View style={[styles.actionBox, styles.actionBoxRed]}>
                      <View style={styles.inProgressRow}>
                        ...
                        <Text style={styles.inProgressText}>Servicio en curso</Text>
                      </View>
                  ...
                  {(canStart || canEnd) && (
                    <>
                      ...
                      >
                        <Text style={styles.btnChatText}>💬 Chat con el jugador</Text>
                      </TouchableOpacity>
                      <TouchableOpacity
                        style={styles.btnMap}
                  ...
                  {isDone && (
                    <View style={styles.completedBox}>
                      <Text style={styles.completedText}>✓ Servicio completado · ${total.toLocaleString()}</Text>
                      {svc.playerRatingGiven ? (
                        <Text style={styles.ratingGiven}>Tu calificación al jugador: {"⭐".repeat(svc.playerRatingGiven)}</Text>
                      ) : (
                        <TouchableOpacity style={styles.btnRate}
                          onPress={() => router.push(`/rating?serviceId=${svc.id}` as any)}>
                          <Text style={styles.btnRateText}>⭐ Calificar al jugador</Text>
                        </TouchableOpacity>
                      )}
                    </View>
                  )}
...
const styles = StyleSheet.create({
  ...
  roleBadge:         { backgroundColor:"rgba(0,255,135,.1)", paddingHorizontal:8, paddingVertical:3, borderRadius:4 },
  roleBadgeText:     { color:"#00ff87", fontSize:10, fontWeight:"700" },
  logoutText:        { color:"#555", fontSize:12 },
  profileBtn:        { width:32, height:32, borderRadius:16, backgroundColor:"#00ff87", alignItems:"center", justifyContent:"center", overflow:"hidden" },
  profileAvatar:     { width:32, height:32, borderRadius:16 },
  profileInitial:    { color:"#0a0a0f", fontWeight:"800", fontSize:14 },
  statsRow:          { flexDirection:"row", gap:10, padding:14, borderBottomWidth:1, borderBottomColor:"#1e1e2a" },
  statCard:          { flex:1, backgroundColor:"#13131c", borderRadius:6, padding:12, alignItems:"center" },
  ...
  offerForm:         { marginTop:14, borderTopWidth:1, borderTopColor:"#1a1a24", paddingTop:14 },
  modeRow:           { flexDirection:"row", gap:8, marginBottom:4 },
  modeBtn:           { flex:1, borderWidth:1.5, borderColor:"#2a2a35", borderRadius:4, padding:9, alignItems:"center" },
  modeBtnText:       { color:"#555", fontWeight:"700", fontSize:11 },
  label:             { fontSize:10, fontWeight:"700", letterSpacing:1.5, color:"#777", marginBottom:6, marginTop:12 },
  ...
  horaBtn:           { flex:1, paddingVertical:9, borderRadius:4, borderWidth:1.5, borderColor:"#2a2a35", alignItems:"center" },
  ...
  infoItem:          { width:"45%" },
  infoKey:           { fontSize:9, color:"#555", fontWeight:"700", letterSpacing:1, textTransform:"uppercase" },
  infoVal:           { fontSize:12, color:"#f0ede8", marginTop:2 },
  actionBox:         { backgroundColor:"rgba(0,255,135,.04)", borderWidth:1, borderColor:"rgba(0,255,135,.2)", borderRadius:6, padding:13, marginTop:10 },
  actionBoxRed:      { backgroundColor:"rgba(255,71,87,.04)", borderColor:"rgba(255,71,87,.2)" },
  actionHint:        { fontSize:11, color:"#666", marginBottom:10 },
  inProgressRow:     { flexDirection:"row", alignItems:"center", gap:7, marginBottom:6 },
  pulseDot:          { width:8, height:8, borderRadius:4, backgroundColor:"#00ff87" },
  inProgressText:    { fontSize:12, color:"#00ff87", fontWeight:"700" },
  btnStart:          { backgroundColor:"#00ff87", paddingVertical:13, borderRadius:6, alignItems:"center" },
  btnStartText:      { color:"#0a0a0f", fontWeight:"800", fontSize:14 },
  btnPayment:        { backgroundColor:"#ffa500", paddingVertical:13, borderRadius:6, alignItems:"center", marginBottom:0 },
  btnPaymentText:    { color:"#0a0a0f", fontWeight:"800", fontSize:14 },
  btnEnd:            { backgroundColor:"#ff4757", paddingVertical:13, borderRadius:6, alignItems:"center" },
  btnEndText:        { color:"#fff", fontWeight:"800", fontSize:14 },
  btnChat:           { borderWidth:1.5, borderColor:"#00aaff", borderRadius:4, padding:9, alignItems:"center", marginTop:10 },
  btnChatText:       { color:"#00aaff", fontWeight:"700", fontSize:12 },
  btnMap:            { borderWidth:1.5, borderColor:"#00ff87", borderRadius:4, padding:9, alignItems:"center", marginTop:8 },
  btnMapText:        { color:"#00ff87", fontWeight:"700", fontSize:12 },
  completedBox:      { borderTopWidth:1, borderTopColor:"#1a1a24", paddingTop:12, marginTop:8 },
  ...
  btnReofert:        { marginTop:8, borderWidth:1.5, borderColor:"#00aaff", borderRadius:4, padding:8, alignItems:"center" },
  ...
  blockedText:       { fontSize:14, color:"#666", textAlign:"center", lineHeight:22 },
