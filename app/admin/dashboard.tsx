...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      setUsers(
        snap.docs.map((d) => ({ id: d.id, ...d.data() }) as PendingUser),
      );
      setLoading(false);
    });
    return () => unsub();
      ...
      <View style={styles.tabs}>
        {TABS.map((t) => (
          <TouchableOpacity
            ...
            style={[
              styles.tab,
              tab === t.key && { borderBottomColor: t.color },
            ]}
            onPress={() => {
              setTab(t.key);
              setLoading(true);
            ...
            {selected && (
              <ScrollView>
                ...
                <Text style={styles.modalSub}>
                  {selected.email} · {selected.ciudad}
                </Text>
                <Text style={styles.modalSub}>
                  CC: {selected.cedula || "—"}
                </Text>
                ...
                {selected.cedulaURL ? (
                  ...
                  <View style={[styles.modalSection, styles.noCedula]}>
                    <Text style={styles.noCedulaText}>
                      ⚠️ No adjuntó cédula
                    </Text>
                  </View>
                )}
