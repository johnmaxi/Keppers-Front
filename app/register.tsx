...
import {
  ...
  View, Alert, ActivityIndicator, Modal, Linking,
} from "react-native";
...
const BANCOS = [
  ...
  "Banco Popular","Scotiabank Colpatria","Banco de Occidente",
  "Banco Caja Social","Nequi","Daviplata","Banco Agrario",
  "Banco Falabella","Banco Pichincha","Bancamía","Otro",
];
...
// ── Dropdown genérico ──────────────────────────────────────────────────────────
function Dropdown({
  label, value, options, onSelect, placeholder, open, onToggle,
}: {
...
export default function Register() {
  ...
  const [form, setForm] = useState({
    ...
    password: "", banco: "", numCuenta: "",
    tipoCuenta: "Ahorros", cedula: "",
  });
  const up = (k: string, v: string) => setForm((f) => ({ ...f, [k]: v }));

  // ── Validaciones por paso ────────────────────────────────────────────────────
  const validateStep1 = () => {
    if (!form.nombre.trim()) { Alert.alert("Error", "Ingresa tu nombre completo"); return false; }
    if (!form.email.trim() || !form.email.includes("@")) { Alert.alert("Error", "Correo inválido"); return false; }
    if (!validarTelefono(form.telefono)) {
      Alert.alert("Error", "Teléfono inválido. Debe ser un celular colombiano de 10 dígitos empezando por 3 (ej. 3001234567)");
      return false;
    }
    if (!form.ciudad) { Alert.alert("Error", "Selecciona tu ciudad"); return false; }
    if (form.password.length < 6) { Alert.alert("Error", "La contraseña debe tener al menos 6 caracteres"); return false; }
  ...
  const validateStep2 = () => {
    ...
    if (form.numCuenta.length > 15) { Alert.alert("Error", "El número de cuenta no puede tener más de 15 dígitos"); return false; }
    if (!/^\d+$/.test(form.numCuenta)) { Alert.alert("Error", "El número de cuenta solo puede contener dígitos"); return false; }
    return true;
  };
...
const styles = StyleSheet.create({
  ...
  selectBtn:         { backgroundColor: "#16161f", borderWidth: 1.5, borderColor: "#2a2a35", borderRadius: 4, padding: 13, flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginTop: 0 },
